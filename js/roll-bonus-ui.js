...
(() => {
  ...
  window.renderChoice = function renderChoice(side, choiceData) {
    ...
    return `<div class="gd-choice gd-choice-panel ${side} ${locked ? "locked" : ""} ${chosen ? "chosen wink-out" : ""}" data-choice-panel="${side}" aria-label="${choiceData.label}"><div class="gd-choice-title">${choiceData.label}</div><div class="gd-choice-mid"><span>⌛ ${choiceData.timeCost}s</span></div><div class="gd-thresholds"><span class="gd-fail">☠ ${thresholds.red}</span><span class="gd-great">♛ ${thresholds.green}</span></div></div>`;
  };
